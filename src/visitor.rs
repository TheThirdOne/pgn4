...
pub enum VisitingError {
    #[error("While trying to follow a step of a partial path, the partial path was wrong in some way.")]
    PartialPathWrong,
    ...
    EmptyTurn,
    #[error("Either the alternative is not present on the last qturn or a qturn has yet to be passed")]
    InvalidAlternative,
...
impl <'a> VisitorMut<'a> {
    ...
    pub fn qturn<'b>(&'b mut self) -> Option<&'b mut QuarterTurn> {
        if self.j == usize::MAX {
            None
        } else {
...
pub trait VisitorCommon where Self: Sized {
    fn last(&self) -> Option<&QuarterTurn>;
    fn next(self) -> Result<Self,VisitingError>;
    fn into_alternative(self, alt: usize) -> Result<Self,VisitingError>;
    fn follow_once<'a>(mut self, partial: &'_ mut PartialPath<'a>, main: &'a [usize]) -> Result<Self,VisitingError> {
    ...
    fn follow_path<'a>(mut self, partial: &'_ mut PartialPath<'a>, path: &'a [usize]) -> Result<Self,VisitingError>{
        if path.len() % 2 == 0 {
            return Err(VisitingError::EvenPath);    
        }
        if path == &[0] {
...
impl <'a> VisitorCommon for Visitor <'a> {
    ...
    fn next(mut self) -> Result<Self,VisitingError> {
        let i = self.i;
        let j = self.j;
        if i == 0 && j == usize::MAX { // special case for the beginning of line
